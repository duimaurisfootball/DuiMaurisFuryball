...
namespace Dui_Mauris_Furyball
{
    public static class FoolBossUnlockSystem
    {
        ...
        public const string ModID = "duimaurisfootballLMAOLMAOLMAO";//CHANGE THIS
        ...
        public static void Setup()
        {
            ...
            AchievementSystem.Setup();
            CleanedData = false;
            IDetour TryBeatBossIDetour = (IDetour)new Hook((MethodBase)typeof(UnlockablesManager).GetMethod(nameof(UnlockablesManager.TryBeatBossWith), ~BindingFlags.Default), typeof(FoolBossUnlockSystem).GetMethod(nameof(FoolBossUnlockSystem.TryBeatBossWith), ~BindingFlags.Default));
            IDetour MainStartIDetour = (IDetour)new Hook((MethodBase)typeof(MainMenuController).GetMethod(nameof(MainMenuController.Start), ~BindingFlags.Default), typeof(FoolBossUnlockSystem).GetMethod(nameof(FoolBossUnlockSystem.Start), ~BindingFlags.Default));
            IDetour CharaStarIDetour = (IDetour)new Hook((MethodBase)typeof(SelectableCharactersSO).GetMethod(nameof(SelectableCharactersSO.PrepareCharacters), ~BindingFlags.Default), typeof(FoolBossUnlockSystem).GetMethod(nameof(FoolBossUnlockSystem.PrepareCharacters), ~BindingFlags.Default));
        ...
        public static void TryBeatBossWith(Action<UnlockablesManager, BossType, EntityIDs> orig, UnlockablesManager self, BossType boss, EntityIDs entity)
        {
            ...
            if (value.unlocks.TryGetValue(boss, out var item))
            {
                ...
                item.AddItem();
                if (value.autoFishPool.ContainsKey(boss)) AddToFishPool(GetItemName(value.unlocks[boss]), value.autoFishPool[boss]);
